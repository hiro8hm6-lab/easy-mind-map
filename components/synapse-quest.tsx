...
function shapePath(shape: NodeShape) { return shape === "star" ? "polygon(50% 0%,61% 34%,98% 35%,68% 56%,79% 91%,50% 70%,21% 91%,32% 56%,2% 35%,39% 34%)" : undefined; }
...
export function SynapseQuest() {
  ...
  const setShape = async (id: string, shape: NodeShape) => { await db.nodes.update(id, { shape, updatedAt: Date.now() }); setShapePickerId(undefined); };
  ...
  const onWheel = (event: React.WheelEvent) => { event.preventDefault(); setViewport((current) => ({ ...current, scale: Math.max(.45, Math.min(1.7, current.scale * (event.deltaY > 0 ? .92 : 1.08))) })); };
