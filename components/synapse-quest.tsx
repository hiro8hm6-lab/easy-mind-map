...
export function SynapseQuest() {
  ...
  const sortedTray = nodes.filter((node) => !node.isStagedInMothership);
  const recallRoots = nodes.filter((node) => node.isStagedInMothership && connections.some((edge) => edge.active && !edge.isCrossLink && edge.sourceId === node.id));
  const mobileCard = recallQueue.length ? recallQueue[mobileIndex % recallQueue.length] : undefined;
